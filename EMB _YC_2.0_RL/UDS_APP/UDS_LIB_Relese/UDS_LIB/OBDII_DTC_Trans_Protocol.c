...
void OBDII_CmdHandler_Rec(ubyte *cmd,ubyte OBDII_lRecDataLength_LO)
{
			...
			||((TP_valContinueServiceValid_MP == 1)&&(OBDII_TP_KeepRecCF ==0))||((OBDII_TP_ContinueFrameState!=0)&&(cmd[1]==0x3e)&&(cmd[0]==0x2)))
	{
		TP_valServiceValid_LO = 0;
			...
			||((TP_valFrameType_LO) == TP_FirstFrame_C)||(((TP_valFrameType_LO) == TP_ControlFrame_C)
			||(((TP_valFrameType_LO) == TP_ControlFrame_C)&&(TP_valContinueServiceValid_MP == 1)&&(OBDII_TP_KeepRecCF ==1)))
...
void OBDII_Neg_Response(void)
{
	...
	ubyte OBD_stNegNoResponseResult=FALSE;
    	...
    	else if( (OBDII_TP_stEffService.valWrongCode!=Incorrect_Message_Length_Or_Invalid_Format)&&(CI_flgDTCFunAddrVaild_MP == TRUE))
    	{
