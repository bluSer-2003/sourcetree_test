...
unsigned char DFlash_BootFLG_Vaild[4] = {0x14,00,00,00};//Need Alig 4
unsigned char DFlash_BootFLG_InVaild[4] = {0x0A,00,00,00};//Need Alig 4
...
//    T_u1            data_length;
    ...
    ///flash_operation(base_addr,data_length,Flash_Data);
    flash_operation(base_addr,data_length,i_p_DataBuff);
    //R_FAD_DF_Read(0xFF2002C0, Data_Flash_Read ,data_length);
...
void FAD_DFlashWriteBootFLG(unsigned char FLG)
{
    /* Defines local variables */
    T_st_HW_Init    hwInit;
    T_u4_FadErr     fadErr;
    T_u4_FadAddr    base_addr;
    T_u1            data_length;
    unsigned char Bootloader_FLG[16] = {0};
    //T_u1            data_num;
   // T_u1            i;
    
    //data[0] = Ali_flash;
    //data[1] = Ali_flag;
    if(*(unsigned char *)(0xFF20FF00) == FLG)
    {
    	return;
    }
    /* Release software protect */
    ...
    checkError(fadErr);
    Bootloader_FLG[0]=FLG;
    data_length = sizeof(Bootloader_FLG); //number of bytes
    ...
    if(FLG == 20)
    {
        flash_operation(base_addr,data_length,DFlash_BootFLG_Vaild);
    }
    else
    {
    	flash_operation(base_addr,data_length,DFlash_BootFLG_InVaild);
    }
...
void flash_operation( T_u4_FadAddr base_addr , T_u1 data_length, T_u4* i_p_DataBuff)
{
    ...
    /* Shifts read mode of BankA */
    fadErr = R_FAD_ShiftToReadMode(R_FAD_GET_FACI_NUMBER(fadErr), R_FAD_NO_TIMEOUT);
    checkError(fadErr);

    /* Blank check BankA Block#1023 (64Bytes) */
    fadErr = sample_DF_BlankCheck(base_addr, base_addr+data_length, R_FAD_INC_MODE, R_FAD_NO_TIMEOUT);
    //checkBlankError(fadErr, R_FAD_STS_BLKCHK0, base_addr+0x40);
    checkBlankError(fadErr, fadErr, base_addr+0x40);

    /* Writes to BankA Block#1023 Offset12 */
    for( count = 0; count < data_length; count = count + 4 )
    {
        fadErr = sample_DF_Write(base_addr+count, (T_p_DataBuff)&i_p_DataBuff[data_count], R_FAD_NO_TIMEOUT);
	    data_count += 1;
        checkError(fadErr);
    }
