...
void OBDII_CallBack_SecurityAccess_Key(ubyte UDS_ValSecLev_LO)
{
	...
	UDS_Valseed.key[0]=0x12;
	UDS_Valseed.key[1]=0x34;
	UDS_Valseed.key[2]=0x56;
	UDS_Valseed.key[3]=0x78;
}
...
ubyte OBDII_CallBack_RoutineCtl_Execute(ubyte Routine_num,ubyte Routine_state,ubyte *ReturnResult_Buff)
{
	...
	switch (Routine_num)
	{
		case 0x00://Angle_Calibration
			...
			{
				Routine_cmdStRLightStar=1;
				*ReturnResult_Buff= Routine_Progressing;
			}
			else if(Routine_state == StopRoutine_C)
			{
				Routine_cmdStRLightStar = 0;
				*ReturnResult_Buff = Routine_Stopped;
			}
			else if(Routine_state== RoutineResult_C)
			{
				//It could be other variables in the program
				if(Routine_cmdStRLightStar == 2)
				{
					OBD_stRoutineCtl1_LO = Routine_Finished_OK;
				}
				else if(Routine_cmdStRLightStar == 1)
				{
					*ReturnResult_Buff = Routine_Finished_NOK;
				}
				else
				{
					*ReturnResult_Buff = Routine_Stopped;
		...
		case 0x01://Cancel_Angle_Calibration
    		...
    		{
    			*ReturnResult_Buff= 1;//routine in progress
    		}
    		*ReturnResult_Buff=2;
    		OBD_stRoutineResultSize_LO=2;
    		return OBD_stRoutineResultSize_LO;
    		break;
		default:
    		return 0;
...
extern void Exsend( exCAN_MSG *pCANMsg,unsigned char Unit1_u08);
extern void can_send(const CAN_MSG *pCANMsg,unsigned char Unit1_u08);
//**************************UDS_Send_Respond****************************//
void OBDII_CallBack_SendAck(ubyte *pcandata,ubyte SendLength)
{
	...
	send_msg.len = 8;
        ...
        {
    	    send_msg.data[i] = pcandata[i];
        }
        for(i=SendLength;i<8;i++)
        {
    	    send_msg.data[i] =0xAA;
        }
